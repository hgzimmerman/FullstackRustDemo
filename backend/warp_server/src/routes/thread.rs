...
pub fn lock_thread() -> BoxedFilter<(impl Reply,)> {
     warp::put2()
         ...
         .and(db_filter())
         .and_then(|uuid: Uuid, _moderator: UserUuid, conn: Conn| {
...
pub fn get_threads_by_forum_id() -> BoxedFilter<(impl Reply,)> {
    warp::get2()
        ...
        .and(db_filter())
        .and_then(|uuid: Uuid, index: i32, conn: Conn| {
            let forum_uuid = ForumUuid(uuid);
...
pub fn get_thread_contents() -> BoxedFilter<(impl Reply,)> {
    warp::get2()
        ...
        .and(db_filter())
        .and_then(|thread_uuid: Uuid, user_uuid: Option<UserUuid>,conn: Conn|{
            let thread_uuid = ThreadUuid(thread_uuid);
