...
pub fn routes(s: &State) -> BoxedFilter<(impl warp::Reply,)> {
    api(&s)
        ...
        .map(|r| {
            ...
            warp::reply::with_header(r, "Access-Control-Allow-Origin", "*")
        })
        .boxed()
}
...
fn api(s: &State) -> BoxedFilter<(impl warp::Reply,)> {
    ...
    warp::path(API_STRING).and(api).with(warp::log(API_STRING)).boxed()
}
...
fn cors() -> BoxedFilter<(impl warp::Reply,)> {
    ...
    warp::options()
        ...
        .map(|_origin: String| {
            let with_header = warp::reply::with_header(warp::reply(), "vary", "origin");
            let with_header = warp::reply::with_header(with_header, "Access-Control-Allow-Headers", "content-type");
