...
fn create_chat(
    new_chat: Json<NewChatRequest>,
    user: NormalUser,
    conn: Conn,
) -> BackendResult<Json<MinimalChatResponse>> {
...
fn add_user_to_chat(
    association: Json<ChatUserAssociationRequest>,
    user: NormalUser,
    conn: Conn,
) -> BackendResult<Json<()>> {
    ...
    Chat::add_user_to_chat(association, &conn).map(Json)
}
...
fn remove_user_from_chat(
    request: Json<ChatUserAssociationRequest>,
    user: NormalUser,
    conn: Conn,
) -> BackendResult<Json<()>> {
    ...
    Chat::remove_user_from_chat(association, &conn).map(Json)
}
...
fn get_chat(chat_id: ChatUuid, _user: NormalUser, conn: Conn) -> BackendResult<Json<ChatResponse>> {
    Chat::get_full_chat(chat_id, &conn).map(ChatResponse::from).map(Json)
}
