...
fn get_users_unpublished_articles(
    logged_in_user: NormalUser,
    conn: Conn,
) -> Result<Json<Vec<MinimalArticleResponse>>, Error> {
...
fn create_article(
    new_article: Json<NewArticleRequest>,
    user: NormalUser,
    conn: Conn,
) -> Result<Json<MinimalArticleResponse>, Error> {
...
fn update_article(
    update_article_request: Json<UpdateArticleRequest>,
    user: NormalUser,
    conn: Conn,
) -> Result<Json<MinimalArticleResponse>, Error> {
    ...
    if article_to_update.author_uuid != user.user_uuid.0 {
        return Err(Error::NotAuthorized {
            reason: "Article being updated does not match the user's id.",
        });
    }
...
fn publish_article(article_uuid: ArticleUuid, user: NormalUser, conn: Conn) -> Result<NoContent, Error> {
    ...
    Article::set_publish_status(article_uuid, true, &conn).map(|_| NoContent)
}
...
fn unpublish_article(article_uuid: ArticleUuid, user: NormalUser, conn: Conn) -> Result<NoContent, Error> {
    ...
    Article::set_publish_status(article_uuid, false, &conn).map(|_| NoContent)
}
